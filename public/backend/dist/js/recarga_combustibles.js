// Recarga_combustibles.js
document.addEventListener('DOMContentLoaded', function() {
    console.log('Recarga_combustibles.js cargado correctamente');
    // ===== BÚSQUEDA EN TABLA =====
    initializeSearch();
    ...
    searchInput.addEventListener('keyup', function(e) {
        ...
        rows.forEach(function(row) {
            try {
                ...
                const cantidad = cells[2].textContent.toLowerCase();
                const tipoCombustible = cells[3].textContent.toLowerCase();
                const precioUnitario = cells[4].textContent.toLowerCase();
                const total = cells[5].textContent.toLowerCase();
                const matches = id.includes(searchTerm) ||
                               fecha.includes(searchTerm) ||
                               cantidad.includes(searchTerm) || 
                               tipoCombustible.includes(searchTerm) || 
                               precioUnitario.includes(searchTerm) ||
...
window.addEventListener('error', function(event) {
    if (event.filename.includes('recarga_combustibles.js')) {
        console.error('Error en recarga_combustibles.js:', event.message, 'en', event.filename, 'línea', event.lineno);
    }
});
