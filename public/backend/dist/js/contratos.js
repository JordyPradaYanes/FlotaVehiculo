    ...
    searchInput.addEventListener('keyup', function(e) {
        ...
        rows.forEach(function(row) {
            try {
                ...
                const fecha_inicio = cells[1].textContent.toLowerCase();
                const fecha_final = cells[2].textContent.toLowerCase();
                const salario = cells[3].textContent.toLowerCase();
                const registradoPor = cells[4].textContent.toLowerCase();
                const matches = fecha_inicio.includes(searchTerm) || 
...
window.addEventListener('error', function(event) {
    if (event.filename.includes('contratos.js')) {
        console.error('Error en contratos.js:', event.message, 'en', event.filename, 'línea', event.lineno);
    }
});