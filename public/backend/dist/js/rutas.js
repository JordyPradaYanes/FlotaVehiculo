// rutas - Funcionalidad de búsqueda y eliminación
    ...
    searchInput.addEventListener('keyup', function(e) {
        ...
        rows.forEach(function(row) {
            try {
                ...
                const nombre_ruta = cells[0].textContent.toLowerCase();
                const descripcion = cells[1].textContent.toLowerCase();
                const distancia = cells[2].textContent.toLowerCase();
                const tiempo_estimado = cells[3].textContent.toLowerCase();
                const costo_peaje = cells[4].textContent.toLowerCase();
                const id = cells[5].textContent.toLowerCase();
                const matches = nombre_ruta.includes(searchTerm) || 
                               descripcion.includes(searchTerm) || 
                               distancia.includes(searchTerm) ||
                                 tiempo_estimado.includes(searchTerm) ||
                                 costo_peaje.includes(searchTerm) ||
    ...
    deleteForms.forEach(function(form, index) {
        form.addEventListener('submit', function(e) {
            ...
            try {
                ...
                const nombreMarca = nombreCell ? nombreCell.textContent.trim() : 'esta marca';
...
window.addEventListener('error', function(e) {
    if (e.filename && e.filename.includes('rutas.js')) {
        console.error('Error en rutas.js:', e.message, e.lineno, e.colno);
    }
});

console.log('Archivo rutas.js completamente cargado');